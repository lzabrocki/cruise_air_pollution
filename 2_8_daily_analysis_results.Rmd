...
data_rank_ci_wd <- data_pair_difference_pollutant_0_wd %>%
  ...
  nest() %>%
  mutate(
    effect = map(data, ~ wilcox.test(.$difference_concentration , conf.int = TRUE)$estimate),
    lower_ci = map(data, ~ wilcox.test(.$difference_concentration , conf.int = TRUE)$conf.int[1]),
    upper_ci = map(data, ~ wilcox.test(.$difference_concentration , conf.int = TRUE)$conf.int[2])
  ) %>%
  unnest(cols = c(effect, lower_ci, upper_ci)) %>%
  select(-data)
...
The output of the matching procedure indicates us the method (1:1 nearest neighbor matching without replacement) and the distance (propensity score) we used. It also tells us how many units were matched: `r sum(matching_ps[["weights"]])`. We assess how covariates balance has improved by comparing the distribution of propensity scores before and after matching: 
...
# save the graph
ggsave(
  graph_propensity_score + labs(title = NULL),
  filename = here::here(
    "inputs",
    "3.outputs",
    "2.daily_analysis",
...
graph_ps_analysis <-
  ...
  geom_pointrange(colour = my_blue, lwd = 1.2) +
