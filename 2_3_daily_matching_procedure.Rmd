...
Our final number of matched treated days is therefore `r nrow(matched_pairs)/2`. We merge the `matched_pairs` with the `matching_matching_data` to retrieve covariate values for the matched pairs and save the data:
...
The distribution of difference in days within a pair is summarized below:
...
We also compute the difference between pairs:
...
a<- treated_pairs %>% 
  ...
  expand(other_date = treated_pairs$date) %>%
...
summarise(
    "Mean" = round(mean(average_difference, na.rm = TRUE), 1),
    "Standard Deviation" = round(sd(average_difference, na.rm = TRUE), 1),
    "Minimum" = round(min(average_difference, na.rm = TRUE), 1),
    "Maximum" = round(max(average_difference, na.rm = TRUE), 1)
  ) %>%
  # print the table
  kable(., align = c("l", rep("c", 4))) %>%
  kable_styling(position = "center")
