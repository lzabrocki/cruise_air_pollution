...
To reproduce exactly the `3_toy_example_randomization_inference.html` document, we first need to have installed:
...
function_randomization_distribution <- function(vector_pair_difference, n_pairs) {
...
distribution_test_statistics <- function_randomization_distribution(vector_pair_difference = observed_pair_differences, n_pairs = 10)
```
...
graph_distribution_test_statistic <- tibble(distribution_test_statistics = distribution_test_statistics) %>%
  ...
  geom_vline(xintercept = average_observed_pair_differences, size = 1.2, colour = my_orange) +
...
# save the graph
ggsave(graph_distribution_test_statistic, filename = here::here("inputs", "3.outputs", "3.toy_example", "distribution_test_statistic_sharp_null.pdf"), 
...
upper_p_value <- sum(distribution_test_statistics >= average_observed_pair_differences)/n_permutations

# compute lower proportion
lower_p_value <- sum(distribution_test_statistics <= average_observed_pair_differences)/n_permutations
...
The two-sided *p*-value for the sharp null hypothesis of no effect is equal to `r round(p_value, 2)`.

# Computing a 95% Fisherian Intervals For Sharp Null Hypotheses
...
# and then compute the average pair difference according 
# to the permuted treatment assignment
function_randomization_distribution <- function(data_adjusted_pair_difference, n_pairs) {
  randomization_distribution = NULL
  n_columns = dim(permutations_matrix)[2]
  for (i in 1:n_columns) {
...
ri_data_fi <- ri_data_fi %>%
  mutate(randomization_distribution = map(data_adjusted_pair_difference, ~ function_randomization_distribution(., n_pairs = 10)))
...
# save the graph
ggsave(graph_p_value_functions, filename = here::here("inputs", "3.outputs", "3.toy_example", "graph_p_value_functions.pdf"), 
       width = 40, height = 20, units = "cm", device = cairo_pdf)
```
...
In Neyman's approach, the inference procedure is built to target the average causal effect. We can estimate $\tau_{\text{fs}}$ with the average of observed pair differences $\hat{\tau}$:

\begin{equation*}
  \hat{\tau} = \frac{1}{J}\sum_{j=1}^J(Y^{\text{obs}}_{\text{t},j}-Y^{\text{obs}}_{\text{c},j}) = \overline{Y}^{\text{obs}}_{\text{t}} - \overline{Y}^{\text{obs}}_{\text{c}}
\end{equation*}

\noindent Here, the subscripts $t$ and $c$ respectively indicate if the unit in a given pair is treated or not. $J$ is the number of pairs. Since there are only one treated and one control unit within each pair, the standard estimate for the sampling variance of the average of pair differences is not defined. We can however compute a conservative estimate of the variance, as explained in chapter 10 of [Imbens and Rubin (2015)](https://www.cambridge.org/core/books/causal-inference-for-statistics-social-and-biomedical-sciences/71126BE90C58F1A431FE9B2DD07938AB):

\begin{equation*}
  \hat{\mathbb{V}}(\hat{\tau}) = \frac{1}{J(J-1)}\sum_{j=1}^J(Y^{\text{obs}}_{\text{t},j}-Y^{\text{obs}}_{\text{c},j} - \hat{\tau})^{2}
\end{equation*}
...
As in the example of [Imbens and Rubin (2015)](https://www.cambridge.org/core/books/causal-inference-for-statistics-social-and-biomedical-sciences/71126BE90C58F1A431FE9B2DD07938AB), we only have here 10 pairs. To build the 95\% confidence interval, we use a $t$-distribution with degrees of freedom equal to J/2-1=9. The 0.975 quantile is equal to `r round(qt(0.975, 9), 3)`. We compute the 95% confidence interval with the following code:
...
squared_difference <- (observed_pair_differences - average_observed_pair_differences)^2

# compute the standard error
standard_error <- sqrt(1/(n_pairs*(n_pairs-1))*sum(squared_difference))

# compute the standard error
ci_lower_95 <- average_observed_pair_differences - qt(0.975, 9)*standard_error
ci_upper_95 <- average_observed_pair_differences + qt(0.975, 9)*standard_error

# store results
neyman_data_ci <- tibble(ci_lower_95 = ci_lower_95, ci_upper_95 = ci_upper_95)
```  
...
function_neyman_t_stat <- function(pair_differences, n_pairs) {
  ...
  # compute the standard error
  squared_difference <- (pair_differences - average_pair_difference) ^ 2
  # compute the standard error
  standard_error <- sqrt(1 / (n_pairs * (n_pairs - 1)) * sum(squared_difference))
...
# compute the observed mean of adjusted pair differences
ri_data_fi_weak <- ri_data_fi_weak %>%
  mutate(observed_neyman_t_stat = map(data_adjusted_pair_difference, ~ function_neyman_t_stat(., n_pairs = 10))) %>%
...
# compute the test statistic distribution
ri_data_fi_weak <- ri_data_fi_weak %>%
  mutate(randomization_distribution = map(data_adjusted_pair_difference, ~ function_randomization_distribution_t_stat(., n_pairs = 10)))
...
```{r}
# define the p-values functions
function_fisher_upper_p_value <- function(observed_neyman_t_stat, randomization_distribution){
  sum(randomization_distribution >= observed_neyman_t_stat)/n_permutations
}

function_fisher_lower_p_value <- function(observed_neyman_t_stat, randomization_distribution){
  sum(randomization_distribution <= observed_neyman_t_stat)/n_permutations
}

# compute the lower and upper one-sided p-values
ri_data_fi_weak <- ri_data_fi_weak %>%
  mutate(p_value_upper = map2_dbl(observed_neyman_t_stat, randomization_distribution, ~ function_fisher_upper_p_value(.x, .y)),
         p_value_lower = map2_dbl(observed_neyman_t_stat, randomization_distribution, ~ function_fisher_lower_p_value(.x, .y)))
```  
...
```{r, layout="l-body-outset", fig.width=10, fig.height=5}
# make the graph
graph_p_value_functions <- ri_data_fi_weak %>%
  select(effect, p_value_upper, p_value_lower) %>%
  rename("Upper p-value Function" = p_value_upper, "Lower p-value Function" = p_value_lower) %>%
  pivot_longer(cols = -c(effect), names_to = "lower_upper", values_to = "p_value") %>%
  ggplot(., aes(x = effect, y = p_value)) +
...
As a point estimate, we take the value of the observed average of pair differences, that is to say `r average_observed_pair_differences`. For this imaginary experiment, our point estimate is close to the true constant effect but the 95% Fisherian interval is wide: [`r ri_data_fi_weak$fi_lower_95`, `r ri_data_fi_weak$fi_upper_95`]. The data are consistent with both large negative and positive constant treatment effects.
