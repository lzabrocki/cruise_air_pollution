...
To reproduce exactly the `2_10_daily_road_traffic_analysis.html` document, we first need to have installed:

* the [R](https://www.r-project.org/) programming language on your computer 
* [RStudio](https://rstudio.com/), an integrated development environment for R, which will allow you to knit the `2_10_daily_road_traffic_analysis.Rmd` file and interact with the R code chunks
...
On average, the hourly road traffic decreases by `r diff_flow` vehicles on weekends. There is a drop of `r round(diff_occupancy, 1)` poins in the occupancy rate.
